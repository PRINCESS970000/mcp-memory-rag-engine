...
def init_internship_tables():
    """جدول التطبيقات الخاص بالتدريب - نفس نمط scholarship_applications."""
...
def check_internship_readiness(student_id: int, role_title: str) -> dict:
    ...
    # الكورسات المكتملة وربطها بالـ skill_tags بتاعتها
    cursor.execute("""
    ...
    courses_ready = skills_ready  # مبسّط: نفس معيار المهارات دلوقتي
    cv_ready = True       # placeholder - لو فيه جدول مستندات لاحقًا يتحدث من هنا
    documents_ready = True  # placeholder - نفس الملاحظة
...
@mcp.tool(
    ...
    description="يحدّث حالة طلب التدريب في internship_applications. "
                ...
                "(اسم مختلف عمدًا عن update_application_state بتاعة المنح "
                "لتجنب تضارب أسماء الأدوات على نفس السيرفر).",
)
